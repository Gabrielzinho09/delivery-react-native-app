...
export const ShoppingBagProvider = ({children}: any) => {
    ...
    const clearShoppingBag = async (): Promise<void> => {
        setShoppingBag([]); // Establecer la bolsa de compras como un nuevo array vacío
        await SaveShoppingBagUseCase([]); // Guardar el array vacío en la persistencia
        setTotal(0) // Establecer el total en cero
      };
